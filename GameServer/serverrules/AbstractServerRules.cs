	...
	public abstract class AbstractServerRules : IServerRules
	{
		...
		public virtual bool IsAllowedToConnect(GameClient client, string username)
		{
            ...
            if ((account == null || account.PrivLevel == 1) && client.TcpEndpointAddress != "not connected")
            {
                ...
                {
                    int bSameIP; bSameIP = 0;
                    if (cln != null && client != null)
                    {
                        if (client != cln 
                            && cln.TcpEndpointAddress == client.TcpEndpointAddress)
                        {
                            if (cln.Account != null)
                                if (cln.Account.PrivLevel > 1)
                                    break;

                            if (cln.Account != null && account != null)
                            {
                                if (cln.Account.Realm != account.Realm)
                                {
                                    client.IsConnected = false;
                                    client.Out.SendLoginDenied(eLoginError.AccountAlreadyLoggedIntoOtherServer);
                                    log.Debug("IsAllowedToConnect deny access; dual realm login not allowed");
                                    return false;
                                }
                            }
                            bSameIP++;
...
/*                    if (bSameIP > 2)
                    {
                        client.IsConnected = false;
                        client.Out.SendLoginDenied(eLoginError.AccountAlreadyLoggedIntoOtherServer);
                        log.Debug("IsAllowedToConnect deny access; Only up to 3 clients are allowed");
                        return false;
                    }
*/
                }
