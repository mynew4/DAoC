...
namespace DOL.AI.Brain
{
	...
	public class CommanderBrain : ControlledNpcBrain
	{
        ...
        {
            if (Body is CommanderPet)
            {
                if (((CommanderPet)Body).bAssist && Owner.TargetObject is GameLiving)
                {
                    if (((Owner.TargetObject) as GameLiving).Realm != Owner.Realm && Owner.TargetObject != Body)
                    {
                        m_orderAttackTarget = (GameLiving)(Owner.TargetObject);
                        Body.TargetObject = (GameLiving)(Owner.TargetObject);
                    }
                }
            }
            base.Think();
