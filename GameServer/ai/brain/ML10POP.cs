...
namespace DOL.GS
{
    ...
    /// New Custom Mob 
    /// </summary>
    public class ML10POPMob1 : GameNPC // on indique ici que le mob fera partie des GameNPC classic
    {
        ...
        private static int ColdHammerID = 166;
        private static int ColdHammerClassID = 22;
        Style sideStyle = SkillBase.GetStyleByID(ColdHammerID, ColdHammerClassID);


        private int dmgmulti = 1; // Variable pour la multiplication des dommages engendr?par le mob
...
//            CurrentSpeed = 0;
//            MaxSpeedBase = 200;
            ChangeBaseStat(eStat.CON, 50);
            ChangeBaseStat(eStat.STR, 30);
            ChangeBaseStat(eStat.DEX, 100);
            ChangeBaseStat(eStat.QUI, 70);
            ChangeBaseStat(eStat.INT, 500);
            ChangeBaseStat(eStat.PIE, 500);
            Mana = 10000;
            RespawnInterval = -1;
        ...
        protected override Style GetStyleToUse()
        {
            ...
            if (this.TargetObject is GameLiving) // on definie la position par rapport a la cible
            {
                ...
                if (Util.Chance(90))
                {
                    ...
                    {
                        Style GetBackStyle = DoBackStyle(living);
                        if (GetBackStyle != null)
                        {
        ...
        /* public ML10POPMob1(INpcTemplate template) // on defini ici l'equipement du mob
            : base(template)
        {
			
		// Attention ! les lignes suivante sont un exemple datant de MIG II ... possible que cela ne s'ecrive plus ainsi aujourd'hui!
			GameNpcInventoryTemplate one = new GameNpcInventoryTemplate();
        	one.AddNPCEquipment(eInventorySlot.Cloak, 3800, 50600);
        	one.AddNPCEquipment(eInventorySlot.TorsoArmor, 2171, 0, 0, 5);
        	one.AddNPCEquipment(eInventorySlot.DistanceWeapon, 2207);
        	Inventory = one.CloseTemplate();
			
          this.SwitchWeapon(GameLiving.eActiveWeaponSlot.Distance);
        ...
        public ML10POPMob1()
            : base()
        {
            Model = 1733; // je decrit pas car ca tu connais
            SetOwnBrain(new ML10POPMob1Brain()); // on attribut l'AI du Mob
        }
        ...
        public override double AttackDamage(InventoryItem weapon)
        {
            if (this.TargetObject != null) // controle securitaire : on verifie que le mob a bien une cible
            {
                if (this.TargetObject is GamePet) // Si la cible est un Pet
                {
                    ...
                    {
                        return base.AttackDamage(weapon) * (dmgmulti * 1);
                    }
            ...
            return (base.AttackDamage(weapon) * dmgmulti); // degats envoy? si : la cible n'est pas un Pet et que le level du mob est superieur a 5 et inferieur a 50
        }
        ...
        public override int MaxHealth  // Maximum de vie du Mob
        {
            get
            {
                if (this.Level <= 10) // Si le mob est level 10 Max :
                {
                    return base.MaxHealth / 2; // on divise sa vie par 2
                }

                if (this.Level >= 50 && this.Level <= 64) // si le mob a un level 50  mini et 64 maxi :
                {
                    return (int)(base.MaxHealth * 1.01); // on multiplira par 5.01
                }

                if (this.Name != this.Name.ToLower() && this.Level >= 65) // si le mob a un level 65 ou plus :
                {
                    return (int)(base.MaxHealth * 8.01); // on multiplira par 8.01
                }
...
// voila , ici on a le Brain Final celui ou le programme ira cherch?les infos qui n'ont pas ??abord?dans les "post-Brain" precedent ... on va y retouver le think , le think interval et tout le merdi?..
...
namespace DOL.AI.Brain
{
    ...
    /// </summary>
    public class ML10POPMob1Brain : APlayerVicinityBrain
    {
        ...
        // ici on declare quelle type de joueur sera la priorit?du mob ... une seul type peut etre prioritaire ... passer sa valeur a 1 et les autres devront rester a zero.
        ...
        public bool viedumob = true;

        /// <summary>
        /// Constructs a new ML10POPMob1Brain
        /// </summary>
        public ML10POPMob1Brain()
            : base()
        {
            m_aggroLevel = 0;
            m_aggroMaxRange = 0;
        ...
        public GameLiving faithPickTarget(ushort radius) // ici on defini la cible
        {
            ...
            foreach (GameNPC npc in Body.GetNPCsInRadius(radius)) // controle de mob present
            {
                ...
                if (GameServer.ServerRules.IsAllowedToAttack(this.Body, npc, false)) // attaquable
                {
                    ...
                    {
                        return npc; // le pet devient la cible (en gros les Pet sont toujours les derniers attaqu? ... )
                    }
                }
            }
            cibler_Pet = 0;
        ...
        {
            ML10POPMob1 MOB = Body as ML10POPMob1; // a partir de maintenant on pourra ecrire MOB au lieu de Body . cela designe dans les deux cas le mob   */
            ...
            if (MOB.Model != 1733) // controle securitaire : Si le mob n'a pas le bon model on lui donne immediatement. (j'avait fait le mob Merlin donc ca devrait etre un gus ^^ )
            {
            }
            ...
            public Spell Nuke
            {
                get
                {
                    ...
                    spell.Damage = this.Body.Level * 10;
                    spell.Name = "The Nuke";
                    spell.Range = 2000;
                    spell.Radius = 200;
                    spell.SpellID = 90015;
            ...
            if (players == 0)
            {
//                Console.WriteLine("plus personne a tuer, je depop");
                Body.Die(this.Body);
            }
            ...
            {
                Body.WalkToSpawn(); // il retourne a son point de depart
                return;
            }
        ...
        /// <summary>
        /// Returns the string representation of the ML10POPMob1Brain
        /// </summary>
        /// <returns></returns>
            ...
            if (living is GamePlayer && aggroamount > 0)
            {
                ...
                foreach (ProtectEffect protect in player.EffectList.GetAllOfType(typeof(ProtectEffect)))
                {
                    ...
                    if (protectAmount > 0)
                    {
                        ...
                        protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(protect.ProtectSource.Client, "AI.Brain.ML10POPMob1Brain.YouProtDist", player.GetName(0, false), Body.GetName(0, false)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
        ...
        protected virtual void BringCloseFriends(AttackData attackData)
        {
            ...
            foreach (GameNPC npc in Body.GetNPCsInRadius(BAFCloseRange))
            {
                ...
                {
                    ML10POPMob1Brain brain = (ML10POPMob1Brain)npc.Brain;
