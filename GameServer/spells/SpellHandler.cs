...
// WHRIA CHANT
                PulsingSpellEffect effect_ = pulsingSpells[pulsingSpells.Count - 1] as PulsingSpellEffect;

                player.DisableSkillKeepDelay((DOL.GS.Skill)effect_.SpellHandler.Spell);
                /*
                DOL.GS.Effects.GameSpellEffect effect = DOL.GS.Spells.SpellHandler.FindEffectOnTarget(Caster, Spell.SpellType);
                if (effect != null)
                {
                    effect.Cancel(false);
                }
		...
		public virtual bool CastSpell(GameLiving targetObject)
		{
			...
			if (Spell.Pulse != 0 && CancelPulsingSpell(Caster, Spell.SpellType))
			{
                ...
                // WHRIA CHANTS
                if (Spell.RecastDelay > 0 && Caster is GamePlayer)
                {
                    (Caster as GamePlayer).DisableSkillKeepDelay((DOL.GS.Skill)Spell);
                }
                // END
			}
			else if (GameServer.ServerRules.IsAllowedToCastSpell(Caster, m_spellTarget, Spell, m_spellLine))
			{
                ...
                // WHRIA stop vampiir sense
                lock (Caster.ConcentrationEffects)
                {
                    for (int i = 0; i < Caster.ConcentrationEffects.Count; i++)
                    {
                        PulsingSpellEffect effect = Caster.ConcentrationEffects[i] as PulsingSpellEffect;
                        if (effect == null)
                            continue;

                        if (effect.SpellHandler.Spell.SpellType=="VampiirEffectivenessDeBuff")
                        {
                            effect.Cancel(false);
                            CancelPulsingSpell(Caster, effect.SpellHandler.Spell.SpellType);
                        }
                    }
                }
...
// WHRIA
// FIX for cheating by pet-inside-wall                                    
                                    Point2D point = Caster.GetPointFromHeading(Caster.Heading, -100);
                                    int prev_x=Caster.X;int prev_y=Caster.Y;
                                    Caster.X = point.X; Caster.Y = point.Y;
                                    playerChecker.Out.SendCheckLOS(Caster, target, new CheckLOSResponse(CheckLOSNPCToTarget));
                                    Caster.X = prev_x; Caster.Y = prev_y;
// END
								}
							}
						}
...
// WHRIA
                if ((target != null || spell.Target.ToLower()=="self") && spell != null && spell.SubSpellID == 0)
				{
					ISpellHandler spellhandler = ScriptMgr.CreateSpellHandler(m_caster, spell, SkillBase.GetSpellLine(GlobalSpellsLines.Reserved_Spells));
...
target.StartInterruptTimer(target.SpellInterruptDuration, ad.AttackType, Caster);
		...
		public virtual AttackData CalculateDamageToTarget(GameLiving target, double effectiveness)
		{
            ...
            // WHRIA
            if (finalDamage == 0 && Util.Random(100) > 50)
                finalDamage = Util.Random(1, 10);
