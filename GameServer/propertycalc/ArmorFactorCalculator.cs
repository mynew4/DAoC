	...
	public class ArmorFactorCalculator : PropertyCalculator
	{
		public override int CalcValue(GameLiving living, eProperty property)
		{
			if (living is GamePlayer)
			{
                ...
                if (bTest)
                {
                    player.Out.SendMessage("/// ArmorFactorCalculator ///", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    player.Out.SendMessage("af=Math.Min((int)(living.Level * 1.875), living.SpecBuffBonusCategory[(int)property]); : " + af, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                //
                ...
                if (bTest)
                {
                    player.Out.SendMessage("af -= living.DebuffCategory[(int)property]; : " + af, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                //
                ...
                if (bTest)
                {
                    player.Out.SendMessage("af += Math.Min(living.Level, living.ItemBonus[(int)property]); : " + af, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                //
                ...
                if (bTest)
                {
                    player.Out.SendMessage("af += living.BuffBonusCategory4[(int)property]; : " + af, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                }
                //
