...
// WHRIA
// NPC skip CanUseStyle                
//                if (!CanUseStyle(living, attackData.Style, weapon))
                if (!CanUseStyle(living, attackData.Style, weapon) && living is GamePlayer)
                {
					if (player != null)
					{
...
//WHRIA
                    if (bTest)
                    {
                        player.Out.SendMessage("///STYLE///", eChatType.CT_System, eChatLoc.CL_SystemWindow);

                        player.Out.SendMessage("living.GetModifiedSpecLevel(Specs.Critical_Strike) : " + living.GetModifiedSpecLevel(Specs.Critical_Strike), eChatType.CT_System, eChatLoc.CL_SystemWindow);

                        player.Out.SendMessage("attackData.Style.GrowthRate : " + attackData.Style.GrowthRate, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("living.GetModifiedSpecLevel(attackData.Style.Spec) : " + living.GetModifiedSpecLevel(attackData.Style.Spec), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("living.AttackSpeed(weapon) : " + living.AttackSpeed(weapon), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("living.UnstyledDamageCap(weapon) : " + living.UnstyledDamageCap(weapon), eChatType.CT_System, eChatLoc.CL_SystemWindow);

                        player.Out.SendMessage("styledDamageCap : " + styledDamageCap, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("factor : " + factor, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        player.Out.SendMessage("attackData.StyleDamage = (int)Math.Max(attackData.Style.GrowthRate > 0 ? 1 : 0, attackData.UncappedDamage * factor);" + attackData.StyleDamage, eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
                    }
...
// WHRIA
                            if (attackData.Style.ID == 244)
                            {
                                procsToExecute.Add(new Tuple<Spell, int, int>(SkillBase.GetSpellByID(22225), 10, 100));
                                procsToExecute.Add(new Tuple<Spell, int, int>(SkillBase.GetSpellByID(21111), 10, 100));
