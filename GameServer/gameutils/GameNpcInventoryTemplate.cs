...
//                            item.UTemplate_Id = "";
        ...
        {
            return AddNPCEquipment_ex(slot, model, color, effect, extension, "", emblem);
        }
        // WHRIA END
        /// <summary>
        ...
        public override bool LoadFromDatabase(string templateID)
        {
            ...
            lock (m_items)
            {
                ...
                {
                    if (!AddNPCEquipment_ex((eInventorySlot)npcItem.Slot, npcItem.Model, npcItem.Color, npcItem.Effect, npcItem.Extension, npcItem.Id_nb, npcItem.Emblem))
                    {
                        if (log.IsWarnEnabled)
                            log.Warn("Error adding NPC equipment for templateID " + templateID + ", ModelID=" + npcItem.Model + ", slot=" + npcItem.Slot + ", id_nb=" + npcItem.Id_nb);
                    }
                }
            }
            return true;
