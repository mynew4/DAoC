...
namespace DOL.GS
{
	...
	public class GameClient : BaseClient
	{
		...
		public void LoadPlayer(int accountindex, string playerClass)
		{
            ...
            {
                if (cln == null || this == cln) continue;
                if (cln.TcpEndpointAddress == this.TcpEndpointAddress)
                {
            ...
            TimeSpan tspan = DateTime.Now.Subtract(dolChar.LastPlayed);
            if (tspan.TotalHours < 1)
            {
                bSameIP = 0;
            }
            if (tspan.TotalHours > 168)
            {
                bSameIP = 255;
            }

            if (dolChar.Region == 163
                || dolChar.Region == 249 
                || dolChar.Region == 238)
            {
                bSameIP = 255;
 ...
 /*           if (bSameIP > 0)
            {
                dolChar.Xpos = dolChar.BindXpos;
                dolChar.Ypos = dolChar.BindYpos;
                dolChar.Zpos = dolChar.BindZpos;
                dolChar.Region = dolChar.BindRegion;
            }
*/
            // END
