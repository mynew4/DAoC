	...
	public class GameKeepGuard : GameNPC, IKeepItem
	{
		...
		public override void Die(GameObject killer)
		{
            ...
            if (killer != this)
            {
                ...
                {
                    if (killer.Realm != this.Realm)
                    {
                        int inArea = this.GetEnemyCountInArea();
                        ...
                        string message = LanguageMgr.GetTranslation(ServerProperties.Properties.SERV_LANGUAGE, "GameKeepGuard.GuardSpam.Killed", this.Name, this.Component.AbstractKeep.Name, inArea);

                        foreach (GameClient thisClient in WorldMgr.GetAllPlayingClients())
                        {
                            //                          if (thisClient.Player.Realm == this.Realm)
                            thisClient.Player.Out.SendMessage(message, messageType, eChatLoc.CL_SystemWindow);
                        }
